...
// GetAllRoutes return all routes registered in this router
func (r *Router) GetAllRoutes() []*route {
	return r.routes
}

// Match return url route and handler name of matched handler
func (r *Router) Match(req *http.Request) (template string, matched bool) {
	template, h := r.find(req)
	return template, h.name == "Matched!"
}
...
func (r *Router) add(method, path string, h http.HandlerFunc, cors *CorsAccessControl, interceptors ...Interceptor) {
	...
	for i, l := 0, len(path); i < l; i++ {
		if path[i] == ':' {
			...
			r.insert(method, path[:i], nil, stype, nil, cors)
			pnames[method] = append(pnames[method], path[i+1:])
...
func (r *Router) find(req *http.Request) (prefix string, hw handlerwrapper) {
	// get tree base node from the router
	cn := r.root

	//todo refactor name string literal to strong type
	hw = handlerwrapper{name: "notFoundHandler", h: notFoundHandler}
